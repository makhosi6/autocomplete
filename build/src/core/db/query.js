...
function getRank(word, query) {
    ...
    if (exact)
        return 0;
    if (starts && singleWord)
        return 1;
    if (starts)
        return 2;
    return 3;
}
function sortByRelevance(words, query) {
    return [...words]
        .map(word => ({
        word,
 ...
 */
async function search(q, limit = 10, sort = 'ASC') {
 ...
 */
async function aggregate(q, limit = 10, sort = 'ASC') {
    const trimmed = q.trim();
    ...
    const escaped = (0, helpers_1.redisEscape)(trimmed);
...
async function suggest(q, limit = 10, sort = 'ASC') {
    const trimmed = q.trim();
    console.log('[Autocomplete][search] Input', { q, limit, sort });
    if (!trimmed) {
        console.log('[Autocomplete][search] Empty trimmed query');
