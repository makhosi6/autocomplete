...
describe('Rate Limit Redis Server Test', function() {
    ...
    t('should make requests until rate limit is reached', async function ()  {
      
		this.timeout(2000);
      
        r (let i = 1; i <= RATE_LIMIT; i++) {
        ...
        et res;
      	...
      	return Promise.reject(err);
			...
			ert.strictEqual('x-ratelimit-limit' in res.headers, true);
        ssert.strictEqual('x-ratelimit-remaining' in res.headers, true);
          sert.strictEqual( parseInt( res.headers['x-ratelimit-limit'] )
           RATE_LIMIT);
        ...
        assert.strictEqual(res.status, 200, 
          ...
          is likely due to running the test multiple times without letting the \
        e expire. Run the test again in ${TIMEFRAME_SEC} second(s)`);
      	assert.strictEqual( parseInt( res.headers['x-ratelimit-remaining'] )
        	, RATE_LIMIT-i );
        
         else {
        assert.strictEqual(res.status, 429);assert.strictEqual( parseInt( res.headers['x-ratelimit-remaining'] )
      			, 0 );
  		}
